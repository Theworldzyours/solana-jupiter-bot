...
async function main(): Promise<void> {
	try {
		...
		try {
			...
			if (
				!fs.existsSync(`${strategyPath}.ts`) &&
				!fs.existsSync(`${strategyPath}.js`)
			) {
			...
			if (!strategyModule.default) {
				console.error(
					`Strategy '${strategyName}' does not export a default strategy`,
				);
				process.exit(1);
			}
			...
			console.log(
				`Strategy '${strategy.name}' initialized: ${strategy.description}`,
			);
...
function setupEventHandlers(
	...
	eventBus.on(EventType.TRADE_COMPLETE, (result) => {
		...
		if (result.success) {
			console.log(
				`Profit: ${result.profit} (${result.profitPercentage.toFixed(2)}%)`,
			);
	...
	eventBus.on(EventType.SIMULATION_COMPLETE, (result) => {
		console.log(
			`Simulation completed: ${result.success ? "Success" : "Failed"}`,
		);
		if (result.success) {
			console.log(
				`Expected profit: ${
					result.expectedProfit
				} (${result.expectedProfitPercentage.toFixed(2)}%)`,
			);
		} else if (result.error) {
			console.log(`Error: ${result.error}`);
		}
