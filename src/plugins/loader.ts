...
export class PluginLoader {
				...
				if (stats.isDirectory()) {
					try {
						...
						if (!fs.existsSync(mainFile) && !fs.existsSync(mainTsFile)) {
							console.warn(
								`Plugin '${item}' does not have an index.js or index.ts file. Skipping.`,
							);
							continue;
						}
						...
						if (
							...
							console.warn(
								`Plugin '${item}' does not export a valid Plugin. Skipping.`,
							);
							continue;
						}
						...
						console.log(
							`Plugin '${plugin.name}' v${plugin.version} loaded from ${itemPath}`,
						);
					} catch (error) {
	...
	public async loadPlugin(pluginName: string): Promise<Plugin | null> {
		try {
			...
			if (!fs.existsSync(mainFile) && !fs.existsSync(mainTsFile)) {
				console.warn(
					`Plugin '${pluginName}' does not have an index.js or index.ts file.`,
				);
				return null;
			}
