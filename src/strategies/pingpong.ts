...
export default class PingPongStrategy implements TradingStrategy {
	...
	public description =
		"Trade back and forth between two tokens when profitable";
	...
	public async execute(
		...
		try {
			...
			const amountInAtomic = Math.floor(
			...
			const outputAmount =
				parseInt(bestRoute.outAmount) / Math.pow(10, outputToken.decimals);
	...
	public async simulate(
		...
		try {
			...
			if (this.direction === "ping") {
				...
				const pongAmountAtomic = Math.floor(
					pongAmount * Math.pow(10, outputToken.decimals),
				).toString();

				// Simulate pong trade (back to original token)
				const pongQuoteResponse = await axios.get<JupiterQuoteResponse>(
					`${this.jupiterApiBaseUrl}/quote`,
					{
						params: {
				...
				if (pongQuoteResponse.data && pongQuoteResponse.data.data.length > 0) {
					...
					const roundTripAmount =
						parseInt(roundTripOutput.outAmount) /
				...
				if (this.lastTrade) {
					...
					expectedProfitPercentage =
						(expectedProfit / this.lastTrade.inputAmount) * 100;
				}
			}
